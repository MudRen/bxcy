#define OTHER_ROOM __DIR__"mengjin-dukou2.c"
...
{
//who = this_player();
        object ob=present("lao zhangduo",this_object());
        who = this_player();
        if (dir=="enter"){
          if (!ob)      return notify_fail("船上几位船夫大声嚷道：老掌舵不在，"+RANK_D->query_respect(who)+"等会再上船罢。\n");
          if (!living(ob))      return notify_fail("船上几位船夫大声嚷道：这位"+RANK_D->query_respect(who)+"，老掌舵就在你身边，先叫醒他罢。\n");
          if ( NATURE_D->outdoor_room())
             return notify_fail("老掌舵一下挡在船前，对不起，晚上过渡太危险了！\n");
          if ( !who->query_temp("rent_paid"))
             return notify_fail("老掌舵一下挡在船前，白眼一翻：嘿嘿！不给钱想过江吗？\n");
        }
