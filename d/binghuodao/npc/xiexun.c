...
destruct(ob2);
destruct(ob3);
tell_object(ob,"谢谢你帮我们回到中土.这本秘籍就送给你吧.我们就此告别.\n");	
//tell_object(ob,"我们就此告别.\n");
//   if(!ob->query("send_zhang"))	
//   {ob->set("send_zhang",1);	
book=new("/clone/book/qishang");        	
book->move(ob);	
...
set_skill("sword", 1240);	
set_skill("blade", 1500);	
set_skill("parry", 1400);	
set_skill("cuff", 1200);	
set_skill("blade", 1500);	
set_skill("jinwu-daofa", 1500);	
set_skill("jiuyang-shengong", 1500);	
set_skill("lingxu-bu", 1500);	
set_skill("damo-jian", 1250);	
set_skill("qishang-quan", 1250);	
...
set("chat_chance",8);
        ...
        setup();
carry_object("/d/binghuodao/npc/obj/killdragon")->wield();  	
