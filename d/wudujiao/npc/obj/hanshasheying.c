...
if(target->query("age") <= 17 && userp(target))       	
return notify_fail("为了世界更美好,放过小孩子吧。\n");
if(target->query_temp("apply/shade_vision")) 
return notify_fail("你看不到"+target->query("name")+"。\n");          	
if(me->query_temp("apply/name")&&userp(target))  
