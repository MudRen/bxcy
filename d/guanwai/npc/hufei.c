...
set_skill("hujia-dao", 1500);       // 胡家刀法          	
set_skill("blade", 1500);             // 基本刀法	
set_skill("hunyuan-yiqi", 800);      // 混元一气功	
set_skill("dodge", 1200);             // 基本躲闪	
set_skill("shaolin-shenfa", 1200);    // 胡家身法	
...
else if((int)(me->query("shen") > 5000000))	
	{
		...
		write(HIC "胡斐说着转身从阎基尸身上搜出一本破旧书本，递了给你。\n");
			...
			add("book_count", -1);
    			...
    			ob->move(this_player());
message("channel:chat", HIY"【镜花水月】"+HIM""+me->query("name")+"拿到胡家刀谱啦。\n"NOR, users());		
