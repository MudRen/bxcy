...
void create()
{
	set_name("九阴真经上卷", ({ "jiuyin shangjuan", "jiuyin", "shangjuan", "book", "shu"}));
	set_weight(600);
	if( clonep() )
		set_default_object(__FILE__);
	else {
		set("unit", "本");
		set("long", "这是武学秘籍《九阴真经》的上卷，上面记述的都是道家修炼内功的大道，以及拳经剑理，并无克敌制胜的真实功夫。相传是宋徽宗年间一位叫黄裳的所作。\n");
		set("value", 50000);
		set("material", "paper");
...
int do_study(string arg)
{
	...
	if (!(arg=="jiuyin shangjuan" || arg == "jiuyin" || arg == "shangjuan" || arg == "book" || arg == "shu"))
		return 0;
...
*/
	...
	cost = 20 + 20 * (25 - me->query("int"))/20;
	if (cost < 10) cost = 10; // minimum cost

	if (arg == "jiuyin" && present("jiuyin xiajuan", me)) {
		...
		if (!random(5)) message("vision", me->name() + "专心致志地研读一本经书。\n", environment(me), me);
