...
{
        int learn;
object thousand_gold;
        learn=who->query_skill("literate",1);
        learn= learn?learn*10:500;
thousand_gold=present("thousand-gold",who);
if (thousand_gold&&thousand_gold->query_amount()>100){
say(this_object()->name()+"受宠若惊地说：“这么多钱我可不敢收呀！”\n");
return 0;}
if(ob->value()>500000000)     
{
say(this_object()->name()+"受宠若惊地说：“这么多钱我可不敢收呀！”\n");    	
return 0;
}
if(who->query("dushu")>5000000)                	
{
...
if ((int)times>99)                   return notify_fail("一次学这么多,小心累到了哦~\n");          	
        if( !living(ob) )
