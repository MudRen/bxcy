 // 此程序采自ldj老版本makelove.c 现已完全修改成豪侠晚歌版本，呵呵
// change by edward@hyl 2001 
...
void over4(object me,object obj)
{
                ...
                me->add("jing",obj->query_con()-55+random(10));
                me->add("makeloved",1);
        if ((random(obj->query("makeloved")) > 3) && (random(me->query("makeloved")) > 3) && (!(obj->query_condition("huaiyun"))) )
        {
        ...
        message("shout", HIR "【家有喜事】："HIM"恭喜 "HIY""+ me->query("name") +"(" + me->query("id") +")"HIM"与"HIY""+ obj->query("name") +"(" + obj->query("id") +")"HIM"做了准父母。\n" NOR,users());
                ...
                obj->apply_condition("huaiyun",12);           
