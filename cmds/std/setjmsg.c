...
int main(object me, string info)
{
        ...
        weapon = present( me->query("jia/id"), me );
        if( !objectp(weapon))
        ...
        switch(command)
        {
                case "-w":
                        ...
                        me->set("jia/wear_msg",msg);
                        weapon->set("wear_msg",msg+"\n");
                        break;
                case "-u":
                        msg = creat_info(msg1,1);
                        me->set("jia/unequip_msg",msg);
...
string creat_info(string msg1,int flag)
{
        string w_name,name;
        int w_lv,color;
        w_lv = this_player()->query("jia/lv");
        w_name = this_player()->query("jia/name");
        color = (w_lv/10);
        color = w_lv - color*10;
        switch(color){
                case 1:
