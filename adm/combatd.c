...
varargs int do_attack(object me, object victim, object weapon, int attack_type)
{
    ...
    if( random(ap + cost) < dp ) {      // Does the victim dodge this hit?
        result += dodge_msg;
        if( ap+cost > dp
        && random(your["jing"]*100/your["max_jing"] + victim->query_int()) < 360 ) {
        ...
        if( random(ap + cost)< pp ) {
            result += parry_msg;
        if( ap+cost > pp
                && random(your["jing"]*100/your["max_jing"] + victim->query_int()) < 350 ) {
                your["combat_exp"] += victim->query_int()/10;
...
// npc的特殊防御
            ...
            if( your["combat_exp"]+1000 > my["combat_exp"]
                &&  (random(my["jing"]*100/my["max_jing"] + me->query_int()) < 350) ) {
