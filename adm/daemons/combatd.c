...
varargs int skill_power(object ob, string skill, int usage)
{
    ...
    if (usage == SKILL_USAGE_ATTACK){
        ...
        power*=((ob->query_dex()/5 + ob->query_temp("dex"))/10);

      power += (ob->query("combat_exp")/10);
    if (power>10000000){
        power /= 100;
        power*=status;
    }else{
...
// npc的特殊防御
           ...
           // else exp get slowest increase
            ...
            {
if(me_exp>1000000000)
                
                  	my["combat_exp"] += me->query_int()/60;
else if(me_exp>500000000)
...
if( (int)victim->query("age")>17||(int)victim->query("combat_exp")>1500000||victim->query_condition("killer"))	
    	{
        ...
        victim->skill_death_penalty();
         ...
         if (stringp(victim->query_temp("die_for"))){
            ...
            sprintf("听说%s(%s)%s死了。", victim->name(1),victim->query("id"),victim->query_temp("die_for")));
...
if (userp(killer) && victim->query_condition("killer"))
                    {
...
money->set_amount((random(5)+1) * 10);
                           money->move(killer);}
                           CHANNEL_D->do_channel(this_object(), "rumor",
sprintf("官府通缉犯%s被%s(%s)就地正法，%s获得了应有的奖励!",victim->name(1),killer->name(1),killer->query("id"),killer->name(1)));
         ...
         if(objectp(killer))
                {
                    ...
                    //    else msg = "在一个不为人知的地方";
                        msg="被"+killer->name(1)+"("+killer->query("id")+")";
    ...
    if ((killer->query_temp("pker_starttime")+900*killer->query_temp("pker_time")) < killer->query("mud_age"))
       {
        ...
        if(!victim->query_condition("killer"))
         CHANNEL_D->do_channel(this_object(), "rumor",
                                sprintf("官府开始捉拿%s(%s),并奖励将杀人犯就地正法之江湖好汉。", killer->name(1),killer->query("id")));
