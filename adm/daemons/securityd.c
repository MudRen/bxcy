...
// trusted.
nosave mapping trusted_read = ([               // ¿É¶Á
    "/":        ({ "(boss)","(arch)","(admin)","(angel)","(genie)","(apprentice)","(wizard)"}), 
    "open":        ({"(immortal)","(ceo)","(president)","(manager)",}),
    "u" :      ({"(ceo)","(president)"}),
]);
nosave mapping exclude_read = ([               //²»ÄÜ¶Á
...
nosave mapping trusted_write = ([
	"/": 		({ "(boss)","(admin)"}),
	"u":            ({  "(arch)","(angel)","(apprentice)","(wizard)","(president)","(ceo)"}),
  "data":         ({ "afei"}), 
