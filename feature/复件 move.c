...
//			env->add_encumbrance(-weight());
			if(userp(env)) tell_object(env,REM0(this_object())); 
			else
                        if(!living(env)&&!env->is_character()) tell_room(env,REM1(this_object()),this_object());
			//else log_file("move.log",ctime(time())+"：
		}
		if (ob)
		{
//			ob->add_encumbrance(weight());
	                if (userp(ob)||interactive(ob)) tell_object(ob,ADD1(this_object())); 
			else
                        if (!ob->is_character()) tell_room(ob, ADD0(this_object()),this_object());
                }
	}
    move_object(ob);
    ob->add_encumbrance(weight());
