...
int perform(object me, object target)
{
        ...
        msg = RED "$N口中轻轻吟唱起了咒文.手中渐渐聚起了一团火球.火球渐大.\n"NOR;
        msg += RED"$N扬起了手,不知不觉中已对准了$n，$n看到漫天火光，抽身后跃，只这一刹，\n"NOR;
        msg += RED"$N大叫一声  火炎球!!  ,手中的火炎直向$n飞去，快捷无伦.！\n"NOR;
           ...
           random(me->query("combat_exp"))>(int)target->query("combat_exp")/3)
        {
                ...
                target->receive_wound("jing", damage/5);
            	  target->add("neli",-100);
        msg += HIR"$n根本没法躲避，一个巨大火球,端端正\n"NOR;
        msg += HIR"正打在$n的身上！$n全身都烧了起来!!\n"NOR;
                me->add("neili", -10);
        ...
        {
                me->start_busy(2);
        msg += HIY"可是$p轻轻一笑，侧身右转.\n"NOR;
        msg += HIY"火球在$p身侧划过，毫发无伤。\n"NOR;
                me->add("neili", -50);
		message_vision(msg, me, target);
        }
