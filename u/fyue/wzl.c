...
int do_name(string target)
{
   ...
   if (userp(dest) && random(100)>1) { me->delete("waiting_target"); return notify_fail(me->name()+"古怪地笑了笑，摇了摇头。\n"); }
    message("vision",
   ...
   set("long","从雾中楼刀削的脸上你几乎看不出任何的喜怒哀乐。\n");
        delete("inquiry");
   command("go west");
    CHANNEL_D->do_channel(me,"sys",sprintf("雇佣"+me->name()+"杀%s",dest->query("name")));
    call_out("do_chase", 1, me);
