...
/*    if(me->query("waiting_target"))
   ...
   if (base_name(environment())!=query("startroom")){
    ...
    CHANNEL_D->do_channel(me,"sys",sprintf("雇佣李四杀%s",dest->query("name")));
    ob=new(__DIR__"obj/paper");
       ob->set("long",sprintf("你打开草纸一看，上面字迹模糊地写着：\nXXXXXXX雇佣***李XX杀X**%sXXXXXX\n\n\n",dest->query("name")));
    ob->move(this_object());
...
{
         int i;
        if (ob->query("money_id") && ob->value() >= 10
        && !this_object()->query("waiting_target")) 
    {
        ...
        set_skill("force", i+400 ); 
        set_skill("unarmed", i+1000 ); 
        set_skill("parry", i+1000 ); 
        set_skill("dodge", i+1000 ); 
        set_skill("sword", i+1000 ); 
         set("combat_exp",(i*i/2)*i*i);
        return 1;
    }
