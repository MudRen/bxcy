...
string *banned_id =   ({"admin","arch","wizard","apprentice","immortal","player","fuck"});
...
write(NOR"                    "+"             "HIM " 夕 阳 再 现\n"NOR );
write(NOR"                    "+"                        "+BGRN HIW"之\n\n"NOR);
write(NOR"                    "+"                 "+HIW"「豪侠晚歌」\n\n"NOR);
...
write(MAG"       ※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※\n"NOR);
write("\n");
write("            "HIC"※"NOR MAG"————————————————————————————"NOR HIC"※\n");
write(MAG"            ｜  "+NOR CYN" 豪侠晚歌"BBLU HIW"信箱"NOR" : "HIR"jhfy@email.com.cn| sgmud@cmmail.com"MAG"   ｜\n"NOR);
write(MAG"            ｜  "+NOR CYN"       豪侠晚歌录"BBLU HIW"论坛"NOR" : "BBLU YEL"JHFY.ABC.YESITE.COM"NOR MAG"           ｜\n"NOR);
write(MAG"            ｜       "+BBLU HIW"豪侠晚歌"NOR CYN"主页"BBLU GRN"HTTP://JHFY.WUDU.NET"NOR MAG"           ｜\n"NOR); 
write(MAG"            "HIC"※"NOR MAG"————————————————————————————"NOR HIC"※\n");
write("\n\n");	
	...
	UPTIME_CMD->main();
	printf(NOR+ HIR"〖"BBLU HIW"豪侠晚歌"NOR HIR"〗"+NOR"自架站起，"+"共有"+HIY"%s"NOR+WHT"人次来访\n"NOR,chinese_number(i_user)); 
	i_user=i_user+1; 
...
printf(NOR+  HIR"〖"BBLU HIW"豪侠晚歌"NOR HIR"〗"+NOR"目前共有"+HIW"%s"NOR+"个注册玩家\n",chinese_number(i_user));
write(NOR+  HIR"〖"BBLU HIW"豪侠晚歌"NOR HIR"〗"+NOR"自开站以来最高在线人数："+HIC+chinese_number(online_num)+NOR" 人\n");
printf(NOR+ HIR"〖"BBLU HIY"北京时间"NOR HIR"〗"+NOR MAG"%s"NOR"\n\n",CHINESE_D->ture_date(ctime(time())));	 
...
※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※
※                                                                    ※
※  一个人物的天赋对于他或她所修习的武艺息息相关。「豪侠晚歌录」中的  ※
...
varargs void enter_world(object ob, object user, int silent)
{
    ...
    if( !silent ) {
	        ...
	        write("★ 您上次光临豪侠晚歌是从："+login_ob->query("last_from") +"\n");
...
if(!(wizardp(user)&&user->query("env/no_login_msg")))
	message("channel:chat", HIR"〖有间客栈〗"NOR+": "+HIC"听说又来了一位叫做"+HIW+user->name()+HIC"的"+str+"\n"NOR,users());
